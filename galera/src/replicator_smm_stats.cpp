...
//    sv[STATS_FC_CSENT            ].value._int64  = stats.fc_csent;
    ...
    /* Marking ist-receiver running doesn't mean ist has started. */
        ...
        && ist_receiver_.current_seqno() != WSREP_SEQNO_UNDEFINED)
    {
        ...
        // why seqno - 1 ?
        // because current_seqno is incremented before apply action.
        // if node fails to apply or take longer to apply the last action then
        // we don't want to report 100% ist done when last write-set apply is
